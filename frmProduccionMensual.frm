...
Private Sub tlbOperaciones_ButtonClick(ByVal Button As MSComctlLib.Button)
  ...
  Case Is = "agregar"
    ...
    If rsStock.EOF Then
      intRes = MsgBox("No existen movimientos, no se puede determinar el mes a importar.", vbApplicationModal + vbInformation + vbOKOnly, "información...")
      Exit Sub
    End If
    ...
    ' confirmo proceso
    intRes = MsgBox("El último período importado es " & strUltimo & ", el período a importar será: " & strNew & ".", vbApplicationModal + vbQuestion + vbYesNo, "procesando stock...")
  ...
  Case Is = "modificar"
    ...
    If lvwGetValue(lvwDatos, "status") <> 0 Then
      a = MsgBox("No es posible modificar la información, primero deberá eliminar el stock por subconcesiones.", vbApplicationModal + vbOKOnly + vbInformation, "Informacion")
      Exit Sub
    End If
