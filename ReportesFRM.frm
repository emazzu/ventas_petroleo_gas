...
Private Sub cmdAceptar_Click()
  ...
  Dim strParcial, strWhere, strORDER As String
  Dim varNombreParam, varNombreCol, varOperadorLogico, varValor, varOrden As Variant
...
'    Dim rptSubRep As CRAXDRT.Report
    ...
    'si tiene subReport
'    If Me.DataSourceSubRpt <> "" Then
      
      'separo nombre de subreport y recordset asociado
'      arrSubRpt = separateText(Me.DataSourceSubRpt)
      
      'recorro array y le paso a cada subReport el recordset
'      For intCuenta = 1 To UBound(arrSubRpt) - 1 Step 2
        
'        Set rptSubRep = rptReport.OpenSubreport(arrSubRpt(intCuenta))
'        Set rs = adoGetRS(arrSubRpt(intCuenta + 1))
'        rptSubRep.Database.SetDataSource rs
'        Set rs = Nothing
...
'    End If
    ...
    For intCuenta = 1 To rptReport.FormulaFields.Count
      If rptReport.FormulaFields(intCuenta).Name = "{@parametrosFiltrados}" Then
        ...
        IIf(Me.DataWhere <> "", "Filtro: " & Replace(Me.DataWhere, "'", ""), "") & IIf(Me.DataOrderBy <> "", ", Orden: " & Me.DataOrderBy, "") & "'"
      End If
    Next
